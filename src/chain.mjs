// Hash-chained, tamper-evident audit — the shared Own Your Stack security
// primitive, VENDORED so strongroom installs from npm alone.
//
// Upstream: askalf/redstamp `src/audit.mjs` @ 27c2685 (MIT, same author).
// strongroom used to import this as `@askalf/redstamp/audit` via a GitHub
// dependency, which made every `npm i @askalf/strongroom` shell out to git and
// reach GitHub at install time — impossible in an air-gapped or git-less CI
// image — for one ~200-line module with no dependencies of its own. A vault you
// cannot install where the secrets live is not a vault, so it is vendored here
// and strongroom ships with zero runtime dependencies.
//
// The on-disk chain format is a COMPATIBILITY SURFACE: redstamp must still be
// able to verify a strongroom audit log and vice versa (they are one stack, and
// `agent-security-stack` reads both). `test/chain-compat.test.mjs` pins the
// chain against vectors computed with redstamp's own implementation — when you
// sync this file with upstream, that test is the gate.
